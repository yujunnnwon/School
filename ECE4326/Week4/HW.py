...
histogram, bin = np.histogram(original_img.ravel(), 256, [0, 256])
...
"""
cdf = np.cumsum(histogram)              # Numpy의 cumsum()을 이용하여 histogram의 CDF를 할당
cdf = (cdf - cdf.min()) * 255 / ((H * W) - cdf.min())
cdf = cdf.astype(np.uint8)              # Numpy array를 uint8의 영상으로 변환
"""
H * W의 영상을 equalization하여 Histogram의 CDF가 8bit인 영상으로 변환
Histogram Equalization의 공식을 이용하여 이를 시행
