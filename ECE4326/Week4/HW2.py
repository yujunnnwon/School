...
def DFT(padded_image):
    ...
    y = n.reshape((N, 1))
    for row in range(M):                                            # x-axis에 대해 1-D DFT를 구현
...
def HPF(image):
    ...
    for u in range(M):
        for v in range(N):
            # Butterworth Highpass Filter... H(u,v) = 1 / {1 + [D0 / D(u,v)]^2n}을 구현
            H[u, v] = 1 / (1 + np.power((D0 / D[np.abs(u - U0), np.abs(v - V0)]), 4))
    return H

# HPF의 연산을 수행하여 H(u, v)를 만듦
hpf = HPF(dft2d)
